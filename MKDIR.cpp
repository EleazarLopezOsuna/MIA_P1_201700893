...
int MKDIR::BusquedaCarpeta(FILE *fp, objetos::superBloque super, int posInodo, Lista ruta[], int n, int pos) {
    ...
    if (encontrado == 0) {
        ...
        if (pos + 1 == n) { //Si lista->siguiente = NULL, quiere decir que ya llego al final
            return (super.s_inode_start + (folder_block->b_content[j].b_inodo * sizeof (objetos::inodo))); //Retorna el inodo de la última carpeta
        } else {
        ...
        if (this->father_aux.block[i] != -1) {
            ...
            for (int j = inicio; j < 4; j++) {
                if (folder_block->b_content[j].b_inodo == -1) {
                    ...
                    new_i.i_uid = logueado; //Usuario propietario: el de la sesión
                    new_i.i_gid = grupoLogueado; //Grupo al que pertenece: el de la sesión
                    ...
                    if (tipoInode == '0') { //Si es de tipo carpeta se crea su primer bloque en el primer apuntador directo
                        ...
                        //Leer el primer bloque del padre para obtener la posición del padre
                    ...
                    return this->sb_aux.s_first_ino; //Devolver la posición del Inodo del archivo o carpeta
                }
            }
        } else {
